...
const generatePhotos = (numPhotos, albums) => {
  return albums.flatMap((album) =>
    [...Array(numPhotos)].map(() => ({
      ...
      id: faker.string.uuid(),
      title: faker.lorem.sentence(),
      small_url: faker.image.urlLoremFlickr({
        width: 150,
        height: 150,
        category: "cats",
      }),
      big_url: faker.image.urlLoremFlickr({
        width: 600,
        height: 600,
        category: "cats",
      }),
...
fs.writeFileSync("./DB/albums.json", JSON.stringify(albums, null, 2));
fs.writeFileSync("./DB/photos.json", JSON.stringify(photos, null, 2));
